...
class PendingList extends Component {
  ...
  componentDidMount() {
    ...
    axios
      ...
      .then(res => {
        // console.log(res);
        let showPendingImageStatus = new Array(pendingList.length).fill(false);
        this.setState({
          pendingList: res.data,
          showPending: showPendingImageStatus
        });
  ...
  onTableId = (id, ecVerStatusTdId, index) => {
    //console.log({ id });
    let spiStatus = this.state.showPending;
    spiStatus[index] = true;
    this.setState({ showPending: spiStatus });
    const config = {
      headers: {
        'x-auth-token': sessionStorage.getItem('x-auth-token')
      }
    ...
    return this.state.pendingList.map((pendingList, index) => {
      ...
      return (
        <tr key={_id}>
          ...
          <td id={ecVerStatusTdId}>
            {this.state.showPending[index]
              ? `Loading...`
              : statusConverter(ecVerificationStatus)}
          </td>
          ...
          <td style={{ textAlign: 'center' }}>
            <button
              type='button'
              onClick={() => this.onTableId(_id, ecVerStatusTdId, index)}
              className='btn btn-primary'
            >
              Verify
            </button>
